...
const ProjectCard = ({ title, main1, main2, bannerImg, source }) => {
    return (
        <div className="p-3 md:p-6 flex flex-col w-80 bg-[#0c0e19] bg-gradient-to-bl from-black to-transparent shadow-xl shadow-slate-900 rounded-2xl hover:scale-102 transition duration-200">
            <img className="p-4 rounded-4xl" src={bannerImg} alt="" />
            <h3 className="px-4 text-xl md:text-2xl font-bold leading-normal">
                {title}
            </h3>
            <p className="px-4 text-sm md:text-md leading-tight py-2">{main1}</p>
            <p className="px-4 text-sm md:text-md leading-tight py-2">{main2}</p>
